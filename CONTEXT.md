...
- **Upload de photo : câblé mais jamais exercé contre un vrai bucket** (voir §12.16). `S3_ENDPOINT`/`S3_BUCKET`/`S3_ACCESS_KEY_ID`/`S3_SECRET_ACCESS_KEY`/`S3_PUBLIC_URL_BASE` non définis dans cet environnement — seul le chemin de repli (URL manuelle) est réellement testé. À vérifier contre un vrai bucket R2/Supabase Storage avant un pilote.
...
- **Les captures d'écran de la landing (`public/screenshots/`, §12.10) se périment.** Ce sont de vraies captures de l'app à un instant donné (menu de démo, tableau de commandes) — un futur changement visuel du dashboard ou du menu public les rendra visuellement en décalage avec le vrai produit. Pas d'automatisation qui les régénère : à refaire manuellement (`Playwright` + jeu de données de démo, voir §12.10) après tout changement notable de ces pages, et certainement avant la Phase A. Régénérées le 28 juillet 2026 sous la nouvelle palette (§12.14) ; se repérimeront au prochain changement visuel notable.
- **La vidéo démo (`public/videos/`, §12.13) a la même limite, en pire** : un enregistrement, pas une capture statique, donc plus long à refaire (script Playwright avec `recordVideo` + repasse `ffmpeg`) si le parcours de commande change visuellement. À régénérer avant la Phase A également, et chaque fois que `/menu/[qrToken]` change d'apparence de façon notable. Régénérée le 28 juillet 2026 (§12.14), même limite pour la suite.
