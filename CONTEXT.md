...
- **La vidéo démo (`public/videos/`, §12.13) a la même limite, en pire** : un enregistrement, pas une capture statique, donc plus long à refaire (script Playwright avec `recordVideo` + repasse `ffmpeg`) si le parcours de commande change visuellement. À régénérer avant la Phase A également, et chaque fois que `/menu/[qrToken]` change d'apparence de façon notable.
