...
export async function GET(req: Request) {
  ...
  if (!isAppleConfigured()) {
    return NextResponse.redirect(new URL(`/${from}?oauth_error=apple_not_configured`, req.url));
  }
  ...
  return NextResponse.redirect(appleAuthorizeUrl(state, new URL(req.url).origin));
}
